...
class RiscvAssembler():
    ...
    def encodeR(self, f7, rs2, rs1, f3, rd, op) -> int:
        return ((f7 << 25) | (rs2 << 20) | (rs1 << 15)
                | (f3 << 12) | (rd << 7) | op)

    def encodeI(self, imm, rs, f3, rd, op) -> int:
        return ((imm & 0xfff) << 20) | (rs << 15) | (f3 << 12) | (rd << 7) | op

    def encodeJ(self, imm, rd, op) -> int:
        imm31 = (imm >> 20) & 1
        imm21 = (imm >> 1) & 0x3ff
        imm20 = (imm >> 11) & 1
        imm12 = (imm >> 12) & 0xff
    ...
    def encodeB(self, imm, rs2, rs1, f3, op) -> int:
        imm31 = (imm >> 12) & 1
        imm25 = (imm >> 5) & 0x3f
        imm8 =  (imm >> 1) & 0xf
        imm7 =  (imm >> 11) & 1
    ...
    def encodeU(self, imm, rd, op) -> int:
        return (imm & 0xfffff000) | (rd << 7) | op

    def encodeS(self, imm, rs2, rs1, f3, op) -> int:
        imm25 = (imm >> 5) & 0x7f
        imm7  = imm & 0x1f
        return ((imm25 << 25) | (rs2 << 20) | (rs1 << 15)
                | (f3 << 12) | (imm7 << 7) | op)

    def encodeRops(self, instruction) -> int:
    ...
    def encodeIops(self, instruction) -> int:
    ...
    def encodeBops(self, instruction) -> int:
    ...
    def encodeSysops(self, instruction) -> int:
        op = instruction.op
        if op == "FENCE": #! TODO
    ...
    def encodeMemops(self, instruction) -> int:
        op = instruction.op
        if op == "DATAW":
    ...
    def encodeDebugops(self, instruction) -> int:
        op = instruction.op
    ...
    def encode(self, instruction) -> int:
        encoded = 0
    ...
    def iFromLine(self, line) -> Instruction:
        line = line.strip()
        if len(line) == 0:
            return None
        if ' ' not in line:
