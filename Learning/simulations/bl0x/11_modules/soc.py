...
class SOC(Elaboratable):
    ...
    def elaborate(self, platform: Platform) -> Module:
        ...
        memory = DomainRenamer("slow")(Memory())    # Also, rename/move the "sync" domain of Memory to "slow"
        cpu = DomainRenamer("slow")(CPU())          # and rename/move CPU too.
        m.submodules.cw = cw
        m.submodules.cpu = cpu
        m.submodules.memory = memory

        # Expose/Export to the public
        self.cpu = cpu
        self.memory = memory

        # A buffer to hold led data
        x1 = Signal(32)
